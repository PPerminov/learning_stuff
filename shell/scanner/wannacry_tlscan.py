...
def parse_options():
    ...
    parser.add_option('-f', '--file', dest='list_ip')
...
"""
    print(message)
    if options.network is None and options.list_ip is None:
        parser.print_help()
        sys.exit()
    return options
...
def scan(host):
    ...
    try:
        ...
        if status == 0xc0000205:
            ...
            print('%s - can nfilenameot detect vulnerable status' % host)
    except:
        print('%s Cant do anything!!!!!!!!!!!!!!' % host)
        pass
...
def main():
    ...
    if scan_network and '/' in scan_network:
        ...
        try:
            with open(filename,'r') as list_for_scan:
                for address in list_for_scan:
                    hosts.append(address.strip())
        except Exception as e:
            print ('File not found')
            sys.exit(6)
        print('start to scan {} hosts...'.format(len(hosts)))
    else :
